...
## 镜像拉取
```shell
docker pull fluxmq/enterprise:2.1.2
```

## 2.1.2 版本说明
FluxMQ 2.1.2 版本在 2.1.1 基础上包含以下更新：
...
```shell
docker run -it -d  -v /app/logs:/app/logs  -p 8080:8080 -p 1883:1883 -p 8883:8883 fluxmq/enterprise:2.1.2
```
...
#### 启动FluxMQ
```shell
docker run -it -d  -v /app/logs:/app/logs /app/config/config.yaml:/app/config/config.yaml  -p 8080:8080 -p 1883:1883 -p 8883:8883 fluxmq/enterprise:2.1.2
```
## License配置

> 配置文件目录：/app/license.base64
```shell
docker run -it -d  -v /app/fluxmq.lic:/app/fluxmq.lic  -v /app/logs:/app/logs /app/config/config.yaml:/app/config/config.yaml  -p 8080:8080 -p 1883:1883 -p 8883:8883 fluxmq/enterprise:2.1.2
```
