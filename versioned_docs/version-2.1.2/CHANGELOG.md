...
- 基于 2.1.1 版本的持续优化与问题修复
- 规则引擎与数据源连接稳定性改进
- 文档与安装包版本同步更新

### 📋 配置相关

- **配置文件**：完整示例见 [配置说明](config/read.md)，包含 `license`、`proxy`、`pool.waitStrategy`、`logLevel` 等。
- **License**：支持 `license.mode: LOCAL`（本地文件）与 `license.mode: FCP`（连接 fmq-admin）；FCP 模式下需配置 [proxy](config/license.md#fcp-模式连接-fmq-admin)。
- **pool**：可选配置 `waitStrategy`（如 BLOCKING、YIELDING）用于高并发调优。

### 🔄 兼容性说明

- 与 2.1.1 版本配置文件和 API 保持兼容
- 建议在测试环境验证后再升级生产环境
